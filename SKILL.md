...
2. Use the narrowest reproducible check first. When stable automation is possible, add a failing test before the fix; otherwise explain why and use a minimal reproduction, read-only diagnosis, or targeted temporary logging. Remove temporary debugging code before delivery.
3. Discuss a complete, executable, low-risk plan before editing. Check existing behavior and modules, including risks not stated in the ticket, so the change does not create logical conflicts.
...
5. Follow discuss, design, implement, validate. Do not modify code until the user explicitly confirms the plan.
6. After implementation, review the changed data flow, sibling callers, regressions, and test results. Fix issues found in the same approved scope, then rerun targeted validation.
7. For complex business logic, cross-module behavior, backend/main-process boundaries, persistence, permissions, or recovery semantics, create or update the relevant `docs/*.md` maintenance documentation with the data flow, boundary, failure recovery, and validation approach.
